...
from util.agent import BountyAgent
...
class wave_dash:
    ...
    def run(self, agent: BountyAgent):
...
class flip:
    ...
    def run(self, agent: BountyAgent, manual=False):
        if self.time == -1:
...
class brake:
    @staticmethod
    def run(agent: BountyAgent, manual=False):
...
class shadow:
    ...
    def get_target(self, agent: BountyAgent, ball_loc=None):
        if ball_loc is None:
            ball_loc = self.get_ball_loc(agent)
...
class retreat:
    ...
    def get_ball_loc(self, agent: BountyAgent, render=False):
    ...
    @staticmethod
    def friend_near_target(agent: BountyAgent, target):
        for car in agent.friends:
            if car.location.dist(target) < 400:
                return True
        return False

    def get_target(self, agent: BountyAgent, ball=None):
        target = None
        if ball is None:
